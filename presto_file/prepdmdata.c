...
int main(int argc, char *argv[])
{
    ...
    // printf("%f, %f, %f\n", cmd->dmstart, cmd->dmend, cmd->dmstep);
    // exit(0);
    ...
    if (RAWDATA) {
        ...
        /*
            ...
            get file_num and other things/ Mhz etc
    ...
    if (!strcmp(idata.band, "Radio") && RAWDATA) {
        ...
        if (cmd->nobaryP) {
            char *tfilenm;
            FILE *outtestfile;
            tfilenm = (char *) calloc(60, 1);
            int dmNum = (int)((cmd->dmend-cmd->dmstart)/cmd->dmstep);
            float dmMin = (float)cmd->dmstart;
            float dmMax = (float)cmd->dmend;
            float dmStep = (float)cmd->dmstep;
            float dm_value = dmMin;
            float sampleTime = (float)s.dt*1e6;
            fileinfo.dmNum = dmNum;
            fileinfo.dmMax = dmMax;
            fileinfo.dmMin = dmMin;
            fileinfo.dmStep = dmStep;
            fileinfo.sampleTime = sampleTime;
            fileinfo.channelNum = numchan;
            fileinfo.outfilename = cmd->outfile;
            sprintf(tfilenm, "%s_DM_All_%d_%d.dm", cmd->outfile, numchan, dmNum);
            outtestfile = chkfopen(tfilenm, "wb");
            writeDmProcessFileHead(outtestfile, 1, sampleTime, numchan, numchan, dmNum, dmMin, dmMax, dmStep, &s);
            int dm_i = 0;
            for (dm_i; dm_i<dmNum; dm_i++){  
                    // Determine our dispersion time delays for each channel 
                    for (ii = 0; ii < numchan; ii++){
                        // dispdt[ii] = delay_from_dm(cmd->dm, tobsf[ii]);
                        dispdt[ii] = delay_from_dm(dm_value, tobsf[ii]);
                        }
                    /*
                        在此获取dm序列信息
                        返回由色散引起的延迟（以秒为单位）
                ...
                chkfwrite(idispdt, sizeof(int), numchan, outtestfile);
                dm_value = dm_value + dmStep;  
    ...
    if (cmd->nobaryP) {         /* Main loop if we are not barycentering... */
        ...
        do {
            if (RAWDATA)
                numread = read_psrdata_change(outdata, worklen, &s, idispdt, &padding,
                                       maskchans, &nummasked, &obsmask, fileinfo);
            else if (useshorts)
